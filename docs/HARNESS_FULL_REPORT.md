...
上述 capability 均以当前工作树中的 case/verifier 回归为准；发布到远程分支前仍需重新运行全量测试。
...
| 变量 | 是否必须 | 含义 |
|---|---:|---|
| `SIM_STUDIO_UE_PROJECT` | 条件必需 | `.uproject` 显式覆盖路径；若已设置 `SIM_HARNESS_WORKSPACE` 且其中存在 `ue/SimulatorWorkspace.uproject`，Harness 自动使用 workspace 工程。 |
...
| `scripts/harness_case_library.py` | 规划完整变量空间、materialize/重渲染选定变量，并给历史 run 建立规范化 case/variant 硬链接视图。 | `python3.13 scripts/harness_case_library.py organize --workspace "$SIM_HARNESS_WORKSPACE"` |
