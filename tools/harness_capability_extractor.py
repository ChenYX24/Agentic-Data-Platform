...
PUBLIC_SOURCE_PATHS = [
    ...
    "capabilities/magnetic_force_field.json",
    ...
    "tests/test_harness_bowling_case_family.py",
    "tests/test_harness_magnetic_verifier.py",
    ...
    "cases/templates/bowling_pin_chain.template.json",
    "cases/bowling/bowling_pin_chain_contact.json",
    "cases/bowling/negative_pin_precontact_motion.json",
    "cases/templates/magnetic_force_field.template.json",
...
CAPABILITY_SPECS: tuple[CapabilitySpec, ...] = (
    ...
    CapabilitySpec(
        ...
        title="Static Scene Placement Preflight",
        pattern_type="FLOW",
        stage_ids=("asset_resolution", "static_scene_layout", "camera_planning", "preflight_validation"),
        keywords=(
            ...
            "support relation",
            "overlap_pairs",
            "initial_overlap_pair",
            "missing_physics_asset_binding",
            "camera_plan",
            "harness_build_static_scene",
        ),
        description="Compile case objects and asset selections into a pre-runtime static layout with object nodes, support relations, overlap checks, physics graph membership, and camera coverage.",
        prompt_moves=(
            "Ask for support surfaces, object positions, scale, camera intent, and which objects are physics-critical.",
            "Separate static placement constraints from dynamic runtime behavior.",
            "Require stable object ids that can become runtime actor ids.",
        ),
        runtime_contract=(
            "Write asset_resolution.json, scene_layout.json, and static_scene_report.json before runtime execution.",
            "Each physics-critical object must have selected asset metadata or an explicit analytic proxy fallback.",
            "scene_layout.json must include object_nodes, support_relations, overlap_pairs, physics_graph, and camera_plan.",
        ),
        verifier_checks=(
            "Object ids are unique.",
            "Physics-critical objects have collider, mass, material, and collision profile.",
            "Initial overlaps, support penetration, and missing camera plans fail before runtime.",
        ),
        failure_modes=(
            "F2_asset_missing when physics-critical assets cannot be bound or proxied.",
            "F3_invalid_initial_physics_state when objects overlap or penetrate support surfaces.",
            "F7_runtime_artifact_incomplete when no camera plan exists.",
        ),
        iteration_moves=(
    ...
    CapabilitySpec(
        ...
        title="Magnetic Attract/Repel Force Field",
        ...
        keywords=(
            ...
            "magnetic_radial_direction_m",
            "magnetized_body",
            "magnetic_source",
            "磁吸",
            "排斥",
        ),
        description="Validate magnetic attraction or repulsion using explicit source/subject/mode/strength labels and source-relative radial trajectory evidence.",
        prompt_moves=(
            "Represent magnetism as a force-field control with source_object_id, magnetic_subject_id, magnetic_mode, and magnetic_strength.",
            "Use attract/repel as radial constraints, not as a visual animation label.",
            "Keep magnetic source and subject object ids stable through runtime and verifier artifacts.",
        ),
        runtime_contract=(
            "Trajectory includes source and magnetized body positions.",
            "Case spec declares magnetic_mode=attract|repel and non-zero magnetic_strength.",
            "Runtime exports enough frames to measure radial distance change relative to the source.",
        ),
        verifier_checks=(
            "Magnetic mode and strength are present.",
            "Attract decreases source-relative radial distance.",
            "Repel increases source-relative radial distance.",
            "Radial displacement stays inside the declared expected range.",
        ),
        failure_modes=(
            "Visible motion exists without magnetic field labels.",
            "Attract/repel direction is reversed.",
            "Radial displacement is too small or too large for the declared field response.",
        ),
        iteration_moves=(
