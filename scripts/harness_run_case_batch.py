...
def run_one_case(
    ...
    *,
    ...
    backend = FallbackBackend() if backend_name == "fallback" else UEBackend()
    verifier = PhysicsVerifier()
    try:
        run_dir = backend.run_case(case, output_root, requested_views=requested_views, render_passes=render_passes, camera_strategy=camera_strategy)
        report = verifier.verify_run_dir(run_dir, write=True)
        status = report["status"]
        failure_type = report["failure_type"]
        backend_report = read_optional_json(run_dir / "ue_backend_report.json")
    ...
    return {
        ...
        "render_mode": render_mode if backend_name == "ue" else "debug_fallback",
...
def main() -> int:
    ...
    output_root = ROOT / args.output_root / f"{case_dir.name}_{args.backend}_{timestamp}"
    ...
    if worker_count == 1:
        rows = [
            run_one_case(index, case_path, backend_name=args.backend, output_root=output_root, requested_views=requested_views, render_passes=render_passes, camera_strategy=args.camera_strategy, render_mode=args.mode)
        ...
        with ThreadPoolExecutor(max_workers=worker_count) as executor:
            futures = [
                executor.submit(run_one_case, index, case_path, backend_name=args.backend, output_root=output_root, requested_views=requested_views, render_passes=render_passes, camera_strategy=args.camera_strategy, render_mode=args.mode)
                for index, case_path in enumerate(case_paths)
            ]
    ...
    summary = {
        ...
        "render_mode": args.mode if args.backend == "ue" else "debug_fallback",
        "cases": rows,
    }
